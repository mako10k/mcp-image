...
const STORAGE_ROOT = path.join(os.homedir(), '.cache', 'ai-image-api-mcp');
...
export async function saveImage(
  base64Data: string,
  info: { prompt: string; model: string; params: Record<string, unknown> }
): Promise<ImageRecord> {
  await ensureStorageDirectories();
...
export function getResourceUri(id: string): string {
  return `resource://ai-image-api/image/${id}`;
}
